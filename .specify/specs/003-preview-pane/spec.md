...
1. **Given** a long MDX document open in the editor, **When** the user scrolls in the editor, **Then** the preview scrolls to within ±5% of the same relative position (proportional scroll ratio).
...
- What happens when compilation takes longer than expected? A loading/compiling indicator is shown after 100ms delay (to avoid flicker for fast compilations).
- What happens when the user pastes a large amount of content? The preview still updates (main thread never blocked); if document exceeds 50,000 characters (~1,000 lines), a non-blocking performance warning is shown.
- What happens when a component referenced in MDX doesn't exist? A clear error message indicates the missing component name. *(Handled by FR-006 compilation errors and FR-022 runtime errors)*
...
- **FR-017**: System MUST enforce Content Security Policy on the preview container: `default-src 'none'; script-src 'self' blob:; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'none'`. *(Aligned with Constitution Article III.5)*
...
- Scroll synchronization uses proportional ratio (editor scroll % → preview scroll %) rather than source position mapping. This provides consistent behavior without MDX-specific source map complexity.
...
- **SC-001**: Users see preview updates within 500ms of their last edit in 95% of cases for documents under 50,000 characters; larger documents degrade gracefully with a performance warning displayed.
